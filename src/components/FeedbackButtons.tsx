...
const FeedbackButtons: React.FC<FeedbackButtonsProps> = ({ isOpen }) => {
  ...
  const handleSendCustomFeedback = async () => {
    ...
    setIsSending(true);
    try {
      await sendFeedback('custom', `\`[MetaGame]\` ${sanitized}`);
    ...
    <div className='border-t border-gray-700 pt-3'>
      ...
      <AnimatePresence mode='wait'>
        {feedback === null && !showCustomInput ? (
          ...
          >
            <button
              className='px-2 sm:px-3 py-1.5 rounded text-xs font-semibold transition-colors bg-gray-700 hover:bg-gray-600 text-white'
